...
export default function TenantCardMobile({ tenant, index, onProfile, onSelect, onEdit, onDelete, formatCurrency }) {
  return (
    <div
      className="bg-white rounded-xl border border-slate-200 p-3 shadow-sm active:bg-blue-50 transition-colors"
    ...
    >
      <div className="flex items-center justify-between mb-2">
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="font-medium text-slate-800 text-sm truncate">{tenant.fullName}</span>
        </div>
      ...
      <p className="text-xs text-slate-500 truncate mb-2.5">{tenant.address}</p>
      <div className="flex gap-1.5" onClick={(e) => e.stopPropagation()}>
        ...
        <Button size="sm" variant="outline" onClick={() => onEdit(tenant)} className="h-8 w-8 p-0 border-blue-300 text-blue-600">
          <Pencil className="w-3.5 h-3.5" />
        </Button>
        <Button size="sm" variant="outline" onClick={() => onDelete(tenant)} className="h-8 w-8 p-0 border-red-300 text-red-600">
