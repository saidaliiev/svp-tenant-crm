...
export function generateReceiptPDF(receiptData, settings) {
  ...
  receiptData.transactions.forEach(t => {
    ...
    if (rasPayment > 0) {
      const rasDesc = t.rasReceived ? 'RAS Payment (Info Only)' : 'RAS Payment (Info Only - NOT RECEIVED)';
      tableData.push([
        formatDate(t.date),
        rasDesc,
        '-',
  ...
  doc.text(
    `Generated by ${settings.systemName} on ${formatDate(receiptData.createdDate)}. For assistance contact ${settings.contactPhone || 'your local SVP office'}.`,
